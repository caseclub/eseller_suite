# bench functions that can be used when setting up erpnext
# Use with extreme caution! These methods delete data without mercy!
...
#--------------------------------------------
#Delete all Advance Payment Ledger Entries
#--------------------------------------------
"""
frappe.call("eseller_suite.eseller_suite.doctype.amazon_sp_api_settings.bench_functions.delete_all_advance_payment_ledger_entries")
"""
# def delete_all_advance_payment_ledger_entries():
#     """Deletes every Advance Payment Ledger Entry in the system."""
#     # Fetch every APLE name
#     aple_names = frappe.get_all("Advance Payment Ledger Entry", pluck="name")
#     total = len(aple_names)
# 
#     if not total:
#         return "No Advance Payment Ledger Entry rows found."
# 
#     # Loop and delete with periodic commits
#     commit_interval = 10000
#     for idx, name in enumerate(aple_names, start=1):
#         # Some ledger tables may have docstatus; make sure it's cancelable if needed
#         try:
#             frappe.db.set_value("Advance Payment Ledger Entry", name, "docstatus", 0)
#         except Exception:
#             # If docstatus field doesn't exist, ignore
#             pass
# 
#         frappe.delete_doc(
...
#             ignore_permissions=True
#         )
#         print(f"[{idx}/{total}] Deleted Advance Payment Ledger Entry {name}")
# 
#         if idx % commit_interval == 0:
#             frappe.db.commit()
# 
#     # Final commit for any remaining changes
#     frappe.db.commit()
#     return f"Deleted {total} Advance Payment Ledger Entry rows."

#--------------------------------------------
#Delete all Stock Reconciliation Entries
#--------------------------------------------
"""
frappe.call("eseller_suite.eseller_suite.doctype.amazon_sp_api_settings.bench_functions.delete_all_stock_reconciliations")
"""
# def delete_all_stock_reconciliations():
#     """Deletes every Stock Reconciliation in the system, in batches."""
#     doctype = "Stock Reconciliation"
# 
#     total = frappe.db.count(doctype)
#     if not total:
#         print("No Stock Reconciliations found.")
#         return "No Stock Reconciliations found."
# 
#     batch_size = 100
#     deleted = 0
#     errors = 0
# 
#     print(f"Found {total} {doctype} documents. Starting deletion in batches of {batch_size}...")
# 
#     while True:
#         # Fetch a batch of names (and docstatus) to delete
#         rows = frappe.db.sql(
#             "SELECT name, docstatus FROM `tabStock Reconciliation` LIMIT %s",
#             batch_size,
#             as_dict=True,
#         )
#         if not rows:
...
#             name = row.name
#             try:
#                 # If submitted/cancelled, flip to draft so force delete will work cleanly
#                 if row.docstatus != 0:
...
#                 )
#                 deleted += 1
#                 print(f"[{deleted}/{total}] Deleted {doctype}: {name}")
#             except Exception as e:
#                 errors += 1
...
#         frappe.db.commit()
# 
#     summary = f"Deleted {deleted} {doctype} docs"
#     if errors:
#         summary += f" with {errors} errors."
#     else:
#         summary += "."
# 
#     print(summary)
